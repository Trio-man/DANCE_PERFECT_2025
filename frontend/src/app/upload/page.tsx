...
export default function UploadPage() {
  ...
  useEffect(() => {
    const loadCms = async () => {
      ...
      else setFaqs(((faqRows ?? []) as unknown) as FaqRow[]);
    };
  ...
  // -------------------------
  // ANALYZE FLOW (Cloudinary + Celery)
  // -------------------------
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const dancerUrl = await uploadToCloudinary(dancerVideo);
      const choreoUrl = await uploadToCloudinary(choreoVideo);

      setStatus('Submitting analysis task...');
      const analyzeRes = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dancer_url: dancerUrl, choreo_url: choreoUrl })
      });

      const { job_id } = await analyzeRes.json();
      setStatus('Analysis queued. Processing...');
      setProgress(0);

      // Polling for progress
      const interval = setInterval(async () => {
        const statusRes = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/status/${job_id}`);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen flex flex-col items-center justify-center px-4 bg-gradient-to-br from-[#d6c1ff] via-[#cde7ff] to-white animate-gradient">
        ...
        <motion.div className="bg-white/70 backdrop-blur-lg border border-white/60 shadow-lg rounded-2xl p-8 relative">
          {loading && (
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm flex flex-col items-center justify-center rounded-2xl z-50">
              <div className="animate-spin h-10 w-10 rounded-full border-4 border-gray-300 border-t-gray-700 mb-3" />
              <p className="text-gray-700 font-semibold">{status}</p>
              <p className="text-gray-500 text-sm mt-1">Progress: {progress}%</p>
            </div>
          )}
          ...
          <div className="flex items-center justify-center gap-3 mb-2">
            {appSettings?.logo_url && <img src={appSettings.logo_url} alt="System Logo" className="h-10 w-10 rounded-lg object-contain border border-white/60 bg-white/60" />}
            <h1 className="text-3xl font-bold text-center mb-0" style={{ color: primaryColor }}>{systemName}</h1>
          </div>
          ...
          {resultUrl && (
            <div className="mt-4 text-center">
              <h3 className="font-semibold mb-2">Result Video:</h3>
              <video src={resultUrl} controls className="mx-auto w-full max-w-lg rounded-lg border border-gray-300" />
            </div>
          )}
